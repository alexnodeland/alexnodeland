...
<Layout>
  ...
  <div className="home">
    ...
    <section className="side-projects">
      <h2>{homepageConfig.sideProjects.title}</h2>
      <p>
        {homepageConfig.sideProjects.description}
      </p>
    </section>

    <section className="press">
      <h2>{homepageConfig.press.title}</h2>
      <div className="press-links">
        {homepageConfig.press.links.map((link, index) => (
          <a key={index} href={link.url} className="press-link">
            {link.text}
          </a>
        ))}
      </div>
