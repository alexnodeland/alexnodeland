...
const ExportButtons: React.FC<ExportButtonsProps> = ({ 
  ...
  const searchResume = (term: string) => {
    ...
    resumeData.experience.forEach((exp, index) => {
      const searchableText = `${exp.title} ${exp.company} ${exp.achievements.join(' ')} ${exp.skills?.join(' ') || ''}`.toLowerCase()
    ...
    resumeData.education.forEach((edu, index) => {
      const searchableText = `${edu.degree} ${edu.institution} ${edu.relevantCoursework?.join(' ') || ''} ${edu.achievements?.join(' ') || ''}`.toLowerCase()
      if (searchableText.includes(searchLower)) {
        results.push({
          id: `education-${index}`,
          type: 'education',
    ...
    // Search skills
    const allSkills = [...resumeData.skills.technical, ...resumeData.skills.soft, ...(resumeData.skills.languages || [])]
          ...
          <div className="search-results">
            {searchResults.map((result) => (
              ...
              >
                ...
                {result.company && <div className="result-company">{result.company}</div>}
                {result.institution && <div className="result-institution">{result.institution}</div>}
